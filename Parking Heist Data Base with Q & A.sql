...
SELECT PL.LOG_ID, P.NAME, PL.CAR_PLATE AS CAR_PARKED, PL.CHECK_IN, PL.CHECK_OUT, PY.PAYMENT_TIME FROM PARKING_LOG PL
JOIN PERSONS P ON P.PERSON_ID = PL.PERSON_ID 
JOIN PAYMENTS PY ON PL.PERSON_ID = PY.PERSON_ID
;

-- PAYMENT DATA
SELECT PL.LOG_ID, P.NAME, PL.CAR_PLATE AS CAR_PARKED, PY.CAR_PLATE AS CAR_PAID_FOR, PY.AMOUNT_PAID FROM PARKING_LOG PL 
JOIN PERSONS P ON P.PERSON_ID = PL.PERSON_ID 
LEFT JOIN PAYMENTS PY ON PL.PERSON_ID = PY.PERSON_ID
;

-- Police statements from people who were present
SELECT PL.LOG_ID, P.NAME, PL.CAR_PLATE AS CAR_PARKED, PY.CAR_PLATE AS CAR_PAID_FOR, S.REASON FROM PARKING_LOG PL
JOIN STATEMENTS S ON S.PERSON_ID = PL.PERSON_ID 
JOIN PAYMENTS PY ON PL.PERSON_ID = PY.PERSON_ID 
JOIN PERSONS P ON P.PERSON_ID = PL.PERSON_ID
;

-- The Imposter
SELECT PL.LOG_ID, P.NAME, PL.CAR_PLATE AS CAR_PARKED, PY.CAR_PLATE AS CAR_PAID_FOR, PL.CHECK_IN, PL.CHECK_OUT, S.REASON FROM PARKING_LOG PL 
JOIN PAYMENTS PY ON PL.PERSON_ID = PY.PERSON_ID 
JOIN PERSONS P ON P.PERSON_ID = PL.PERSON_ID 
JOIN STATEMENTS S ON S.PERSON_ID = PL.PERSON_ID 
